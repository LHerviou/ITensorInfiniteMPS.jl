...
function coeff_Coulomb_cylinder_integrand_LL0(x; dpL=1, N=0, dn=0)
  return besselix.(N, x .^ 2 * 2 / dpL^2) .*
         exp.(-dn^2 * dpL^2 * x .^ 2 ./ (2 * x .^ 2 .+ 1)) ./ sqrt.(2 * x .^ 2 .+ 1)
end
...
function build_coefficient_Coulomb_cylinder_LL0(Ly::Float64, N_phi::Int64)
  ...
  for m_1 in 0:(N_phi - 1)
    for m_2 in (m_1 + 1):(N_phi - 1)
      for n_1 in max(0, m_1 + m_2 - N_phi + 1):min(N_phi - 1, m_1 + m_2)
        n_2 = m_1 + m_2 - n_1
        if n_1 >= n_2
          continue
        end
        ...
        if !haskey(precomputation, (N, dn))
          precomputation[(N, dn)] =
            ...
            exp(-N^2 * dpL^2 / 2) *
            hcubature(
              x -> coeff_Coulomb_cylinder_integrand_LL0(x; dpL=dpL, N=N, dn=dn),
              [0.0],
              [10 * Ly],
            )[1][1]
        end
...
function build_coefficient_Coulomb(;
  r::Float64=1.0,
  Lx::Float64=-1.0,
  Ly::Float64=-1.0,
  N_phi::Int64=10,
...
function build_two_body_coefficient_pseudopotential_TI(;
  r::Float64=1.0,
  Lx::Float64=-1.0,
  Ly::Float64=-1.0,
  N_phi::Int64=10,
...
function build_three_body_coefficient_factorized_cylinder(Lx, Ly, N_phi; prec=1e-12)
  ...
  for g in 0:2
    for (k, q) in
        Iterators.product((-3N_phi - g):3:(3N_phi - 1), (-3N_phi - g):3:(3N_phi - 1))
      coefficients[(k, q)] =
    ...
    for (xk, k) in enumerate((-4N_phi - g):4:(4N_phi - 1))
      for (xq, q) in enumerate((-4N_phi - g):4:(4N_phi - 1))
        ...
        for (xr, r) in enumerate((-4N_phi - g):4:(4N_phi - 1))
          ...
          coefficients[(k, q, r)] =
            prefactor *
            W_polynomial(k / 4, q / 4, r / 4, -k / 4 - q / 4 - r / 4) *
            expFactor[xk] *
            expFactor[xq] *
            expFactor[xr] *
            exp(-2 * pi^2 / Ly^2 * (k / 4 + q / 4 + r / 4)^2)
        end
      end
    end
  end
...
function build_hamiltonian_from_four_body_factorized_streamlined_dictionary(
  coeff, N_phi; global_sign=1, prec=1e-12
)
  full_coeff = Dict{Array{Int64,1},Float64}()
  for R4 in 6:(4N_phi - 10)  #Barycenter
    for n_1 in max(0, R4 - 3N_phi + 6):min(N_phi - 4, (R4 - 6) ÷ 4)
      for n_2 in max(n_1 + 1, R4 - n_1 - 2N_phi + 3):min(N_phi - 3, (R4 - n_1 - 3) ÷ 3)
        for n_3 in
            max(n_2 + 1, R4 - n_1 - n_2 - N_phi + 1):min(
          N_phi - 2, (R4 - n_1 - n_2 - 1) ÷ 2
        )
          n_4 = R4 - n_1 - n_2 - n_3
          for m_1 in max(0, R4 - 3N_phi + 6):min(N_phi - 4, (R4 - 6) ÷ 4)
            for m_2 in
                max(m_1 + 1, R4 - m_1 - 2N_phi + 3):min(N_phi - 3, (R4 - m_1 - 3) ÷ 3)
              for m_3 in
                  max(m_2 + 1, R4 - m_1 - m_2 - N_phi + 1):min(
                N_phi - 2, (R4 - m_1 - m_2 - 1) ÷ 2
              )
                m_4 = R4 - m_1 - m_2 - m_3
                temp =
                  global_sign * (coeff[n_1, n_2, n_3, n_4]' * coeff[m_1, m_2, m_3, m_4])
                if abs(temp) > prec
...
function build_four_body_pseudopotentials(;
  r::Float64=1.0,
  Lx::Float64=-1.0,
  Ly::Float64=-1.0,
  N_phi::Int64=10,
  ...
  return build_hamiltonian_from_four_body_factorized_streamlined_dictionary(
    coeff, N_phi; global_sign=global_sign, prec=prec
  )
end
